 ...
 * echo divider(15, 'my-class another-class', 100); // <div class="my-class another-class" style="height: 15px; width: 100%;"></div>
 * echo divider(15, 'my-class another-class', null, 250); // <div class="my-class another-class" style="height: 15px; width: 250px;"></div>
 */
if ( ! function_exists( 'divider' ) ) {
    function divider($height = 2, $class = '', $width_percent = null, $width_px = null) {
        ...
        // Se è un array, sanifica ogni elemento e li unisce con uno spazio
        if (is_array($class)) {
            $class = implode(' ', array_map('sanitize_html_class', $class));
        } else {
            // Se è una stringa, separa per spazi e sanifica ogni classe individualmente
            $class = implode(' ', array_map('sanitize_html_class', preg_split('/\s+/', $class)));
        }
