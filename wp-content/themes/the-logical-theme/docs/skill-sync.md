...
This document explains how to sync a repository-local skill to the global Codex skills directory.

## Purpose

Some Codex environments discover available skills from `~/.codex/skills` instead of using only the repository copy.

This theme provides a sync utility so the repository version of a skill can be treated as the source of truth and then copied into the global Codex skills directory when needed.
...
- writes it into a target Codex skills root
...
Because these are defaults, the script works out of the box for theme-local skills.
...
- Run the sync script after changing a repository-local skill that also needs to exist in `~/.codex/skills`.
